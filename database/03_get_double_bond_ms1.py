...
def get_ozonolysis_delta_mass(metadata: dict):
    ...
    if len(matches) == 0:
        return {
            "chain_shorthand": chain_shorthand, 
            "db_shorthand": lipid, 
            "delta_mass":[], 
            "fatty_acid_pos": [],
            "fragment_type": []
    ...
    for s in matches:
        ...
        for db in pos_db_omega:
            ...
            fatty_acid_pos_list.extend([db, db]) # List[int]
            fragment_type_list.extend(["aldehyde", "criegee"])
    ...
    return {
        ...
        "delta_mass": mz_shift_list,
        "fatty_acid_pos": fatty_acid_pos_list,
        "fragment_type": fragment_type_list,
    }
        

if __name__ == "__main__":
    in_file = "clean_data/lipidmaps_chain_shorthand_with_smiles.csv"
    out_file = "clean_data/ozonolysis_delta_mass.json"

    metadata_list = pd.read_csv(in_file).to_dict("records")
    delta_mass_list = chunked_parallel(metadata_list, get_ozonolysis_delta_mass, chunck_size=1000)
    
    delta_mass_list = [
        record
        for record in delta_mass_list
        if record.get("delta_mass", True)
    ]
    
    with open(out_file, "w") as f:
        f.write("[\n")
        for i, rec in enumerate(delta_mass_list):
            f.write("\t")
            json.dump(rec, f, ensure_ascii=False, separators=(",", ":"))
            f.write("\n" if i == len(delta_mass_list) - 1 else ",\n")
        f.write("]\n")